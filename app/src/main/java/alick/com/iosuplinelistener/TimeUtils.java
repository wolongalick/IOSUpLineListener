...
public class TimeUtils {
    ...
    public static final String format16 = "yyyy年MM月dd日 HH:mm";
...
//        long currentTime = parseStringToMillis("2018-01-10 12:00:00",format1);
    ...
    public static String getYunXinTime(long milliseconds){
        Calendar otherCalendar = Calendar.getInstance();
        otherCalendar.setTimeInMillis(milliseconds);
        ...
        if(otherCalendar.get(Calendar.YEAR)==currentCalendar.get(Calendar.YEAR)){
            //是同一年
            int dayDifference = currentCalendar.get(Calendar.DAY_OF_YEAR)-otherCalendar.get(Calendar.DAY_OF_YEAR);
            if(dayDifference>1){
                //N天以前
                return parseLongToString(milliseconds,format15);
            }else if(dayDifference==1){
                //昨天
                return "昨天 "+parseLongToString(milliseconds,format8);
            }else{
                //是同一天
                int hourDifference = currentCalendar.get(Calendar.HOUR_OF_DAY) - otherCalendar.get(Calendar.HOUR_OF_DAY);
                if(hourDifference >=1){
                    //N小时以前
                    return hourDifference+"小时以前";
                }
                int minuteDifference = currentCalendar.get(Calendar.MINUTE) - otherCalendar.get(Calendar.MINUTE);

                if(minuteDifference>=1){
                    //N分钟以前
                    return minuteDifference+"分钟以前";
                }
                return "刚刚";
            }
        }else {
            return parseLongToString(milliseconds,format4);
        }
    }
